...
- 状态：P1-P14 处置见 v2.1 迭代计划（2026-08-07）——P1-P11/P13 已修复（✓），P12 维持现状（推迟），P14 已在能力测试阶段修复；LLM 节点 / OAG-lite / 动作语义 / ER 增强 / 引擎适配 / 模型可替换等迭代边界推迟至 v2.2（见 paip-vs-palantir 对比报告 §三/§四）
...
**P1. 模板 state/config.json 缺 `exec` 步骤（未随 v2.0 同步）**【v2.1 已修复】
...
**P2. exec 的 `--transform` / `--merge` 过滤参数语义错误**【v2.1 已修复】
...
**P3. `validate --stage` 在流水线早期完全不可用**【v2.1 已修复】
...
**P4. demo-data 金额脏格式与清洗规则不匹配（演示"洗不干净"）**【v2.1 已修复】
...
**P5. schema-infer 对脏日期/金额推断为 `mixed` 且无上下文**【v2.1 已修复】
...
**P10. skill 层缺少"日期清洗标准做法"知识**【v2.1 已修复】
...
**P14. 日期清洗规则曾存在"值被篡改"缺陷（已修复）+ 产物缺少值校验**【已在能力测试阶段修复（0e479e6）】
