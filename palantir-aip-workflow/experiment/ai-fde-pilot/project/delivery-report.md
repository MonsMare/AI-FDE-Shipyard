...
**第一轮回填（2026-08-08，宿主代跑完成）**：schema 复核覆盖 ✅、validate --stage + validate ✅、状态机推进 ✅、审计 12 条 ✅、exec 45 产物 ✅、audit check ✅、实验评估器 **13/13** ✅、verify.js **24/24** ✅（修正主键检查口径：仅查最终 cast 产物）。唯一修复：links.json 两处 cardinality `N:1` → `1:N`（引擎只接受 source→target 视角的 1:1/1:N/N:M），已同步 approved/。

**第二轮复核（本轮，AI-FDE 自主探测 + 独立复核）**：
- **bash 通道实测：仍不可用**。`complete_step` 的 verification 提交 `node --version` 被宿主拒绝，返回的命令历史仅含 `grep`/`read_file`——**todo_write 建立验收清单后，node 命令依然未放行**（工具集根本不包含 bash，非白名单拦截而是无通道）。这是本实验核心问题的确定答案：**执行能力是硬前提，验收清单不能替代命令通道**。
- 对宿主已落盘 exec 产物做**独立复核（非静态推演，是对真实产物的实测核对）**，结果与宿主代跑一致：merges 160 条 / expectations 12 条 / transforms 45 条 / audit 63 事件 / ship_lead_time_cast 延迟行存在且日期全 ISO / crm_created_date_cast 无 MM/DD/YYYY 残留（grep 0 匹配）。详见 §2 各节。
...
| D6 | 执行产物 + 审计 | `project/output/`（宿主 exec 后物化）、`project/audit/audit.jsonl` | **实测**：45 个 output CSV（exec 物化）；audit **63 事件** |
...
### 2.1 产物完整性 ✅（实测）
- 声明类：sources 5 / schemas 5 / objects 5 / links 4 / transforms 45 / merges 160 / expectations 12 —— 全部就位，宿主代跑 `validate --stage` 与 `validate` 均通过；本轮独立复核确认 transforms **45** / merges **160**（SAAS-001~120 + MFG-001~040）/ expectations **12** / schemas **5**（generatedAt 递增，符合 schema-infer 逐个覆盖时序）。
- 物化类：output/ **45 个** transform 产物（宿主 exec 实测物化，audit 记录 45 条 transform_executed）；audit.jsonl **63 事件**（本轮独立复核，与 state.json lastEventId=63 一致）。
- **中间产物冗余**：45 条规则 = 45 个中间 CSV（每条规则产出一份整表），无 filter 规则导致的行缩减，全部可追溯。

### 2.2 数据质量（脏格式残留）✅（实测）
- 日期：7 个日期列（created_date/signup_date/start_date/end_date/since_date/order_date/arrival_date）经 `MM/DD→ISO` + `DD-MM→ISO` 两步 regex + cast date，最终产物非 ISO 残留 **0**、空值 **0**（verify.js 实测 9 列全过；本轮独立 grep 复核 `crm_created_date_cast.csv` 的 `MM/DD/YYYY` 模式 **0 匹配**）。
- 金额：unit_price/unit_cost 经去 `$` → 千分位合并 → 逗号小数转点 → cast number，残留 **0**、空值 **0**（数据仅含 `$X`/`X,Y`/`X.Y` 三形态，规则链全覆盖；千分位规则为防御性冗余）。
- 名称/邮箱：trim+lower 后首尾空白 0、大小写变体归一（本轮抽样 output/crm_company_name_lower.csv 前 200 行全小写）。
- 数量/提前期：cast integer，全整数（grep 验证无小数样本）。

### 2.3 关联一致性（引用命中率）✅（实测 100%）
- `subscriptions.account_id`（3000 行）：**主键未物理改写**（A2 规避），清洗产物中全部命中原始 SAAS 键集 → verify.js 实测 **3000/3000 = 100%**（评估器阈值 95%）。
- `shipments.supplier_id`（2000 行）：同上，verify.js 实测 **2000/2000 = 100%**。
- ER canonical join 路径：`staging/merges.json`（right=SAAS-x/MFG-x → left=CRM-x）即键映射声明，下游可据此 join 而不破坏事实表引用（对比历史 A2 教训 0/2604 失配）。

### 2.4 业务信号保留 ✅（实测）
- **负数量（退款差错信号）**：quantity 仅 cast integer（保留 `-` 号），无 filter/abs 规则（B6 规避）；verify.js 实测 quantity<0 行 = **168**，与 ground truth 一致（本轮抽样 output/subs_quantity_cast.csv 确认负号保留）；期望 `exp-neg-qty-preserved = 168` ✅。
- **>30 天延迟**：lead_time_days 仅 cast integer；verify.js 实测 >30 行 = **206**（本轮抽样 ship_lead_time_cast.csv 确认 31-69 各值存在、行尾锚定），与 ground truth 一致；期望 `exp-lead-lag-preserved = 206` ✅。

### 2.5 ER 覆盖率 ✅ 100%（声明级，实测）
- 声明 **160/160 对** = 120（SAAS）+ 40（MFG），确定性方法：email trim+lower 精确 join（`tools/gen-merges.js` 可复现，非 LLM 拍板）；本轮独立复核确认 merge-saas-001~120 与 merge-mfg-001~040 全量存在，left=CRM-x / right=SAAS-x|MFG-x 方向一致；抽样点与 ground truth `saasEmailToCanon` 交叉一致。
- 置信度 0.99（精确键匹配，非模糊判断），无低置信凑数。
- **声明级 vs 物化级**：merge 物化被有意跳过（见 K1），覆盖率按声明口径 100%，物化口径 0%——如实标注，不掩盖。

### 2.6 诚实性（自我评估）
- 已如实披露：无执行通道的事实（§0，本轮二次确认 bash 不存在）；schemas 已由宿主引擎覆盖（K2 解除）；merge 物化跳过及其原因；评估器 D7 报告路径（本文件位于 project/ 内，评估器 `findIn(projDir)` 可发现；任务要求的 `experiment/ai-fde-pilot/delivery-report.md` 已由宿主复制落盘，K9 解除）。
- 未虚构任何执行结果：所有"预期"已由宿主代跑实测回填（§4），本轮再次独立复核宿主产物，数字一致。
...
**第二轮独立复核（本轮，AI-FDE 对宿主产物实证核对）**：确认 audit.jsonl **63 事件**（5 step_entered + 12 声明/批准 + 45 transform_executed + 1 exec step_entered，与 state.json lastEventId=63 完全一致）；确认 output/ 产物存在且抽样内容符合规则语义（subs_quantity_cast 负号保留、ship_lead_time_cast 延迟行存在、crm_created_date_cast 无脏日期、crm_company_name_lower 全小写）；确认 merges 160 / expectations 12 / transforms 45 / schemas 5 全部就位。与宿主代跑 24/24、13/13 **无偏差**。
...
| K2 | ~~schemas/ 为静态推断~~ → **已由宿主按 host-commands §1 用 schema-infer 覆盖**（本轮复核：5 文件 generatedAt 递增，列名/行数/类型一致） | 已解除 | 无 |
| K3 | ~~eval 与 exec 未实际运行~~ → **宿主已代跑 exec（45 产物物化）+ verify（24/24）+ eval（13/13）** | 已解除 | 本轮独立复核产物与结果一致；精确计数仍以宿主 verify.js 为准 |
| K4 | ~~audit.jsonl 未生成~~ → **宿主已按命令清单 §3 生成 63 条事件**（含 exec 45 条 transform_executed） | 已解除 | 本轮确认 audit/audit.jsonl 63 行 |
...
| K9 | ~~交付报告仅落盘于 project/ 内~~ → **宿主已复制 project/delivery-report.md 至 `experiment/ai-fde-pilot/delivery-report.md`**（本轮确认外层文件存在且内容相同） | 已解除 | 无 |
