...
**我们**：`paip-model` 输出 9 种权威规则（§3.6），`bin/exec.js` 纯 JS 行级执行。**硬约束**：skill 禁止重新实现规则逻辑（防第二真相源）。
...
**我们**：`bin/exec.js`——内部强制先校验（validate），9 种规则行级执行（实验确认语义），合并键映射替换，统一写盘 `output/<id>.csv`，成功一次性审计。**铁律：失败中止、数据不动、状态不推进**。
