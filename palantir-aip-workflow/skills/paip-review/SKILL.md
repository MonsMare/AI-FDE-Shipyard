...
### 2. 逐类呈现给用户（一次性呈现代价最小）
- 转换类待审项：呈现前对每条转换先跑 eval 展示效果对照，供用户基于效果裁决：
```
node <插件根>/bin/eval.js <项目目录> <规则id>
```
  eval 在项目临时副本上执行该单条规则（零副作用），输出执行前后行数/空值数/样本对照（`changes.rowDelta`/`emptyDelta`；`dirtyDelta` 初版为 null，v2.2 补脏模式检测）。规则在 approved 或 staging 均可定位（CLI 自动回退）；eval 失败（如规则引用的列不存在）时把 `✘` 问题一并呈现
